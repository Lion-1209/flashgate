...
| 5 | on-board identity ≠ repo state (git sha or board name) — or the identity evidence channel could not be reset: a failed pre-start signature wipe fails closed (exit 5, start withheld), because a surviving old-boot signature must never count as a pass |
