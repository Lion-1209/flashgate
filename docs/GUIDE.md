...
编译、烧录、听板子说话、跑探针，全过才放行。探针需要串口：0.4.2 起，
档案里声明了探针而串口不可用时，验证直接失败（退出码 6）而不是像
早先那样静默跳过探针。**只接 ST-Link、不接串口的实验台，hook 会拦
（同一棵树最多拦两次，之后放行并警告，见下）**。想让它以纯 SWD 启动
门工作，出路是把 hook 命令里的 --all-probes 去掉（改 .claude/
settings.json 那行命令）——注意只删档案里的探针没有用：守卫在探针
存在性检查之前就生效，删了探针连"串口在"的情况也会失败。
...
**不要在用户级和项目级同时注册**。两个级别都挂着 Stop hook 时，同一次
停止事件会触发两条 hook 并发跑验证，抢同一个串口——这是我们实际踩过的
坑：输家报一个误导性的环境错误（退出码 6，"打不开 COM3"），固件本身
明明没问题。现在 verify 自带台架锁（锁文件在固件目录的
`.flashgate/verify.lock`，进程级互斥，进程死亡时由操作系统释放，不会留
陈旧锁）：并发的第二个验证会**等待**第一个跑完再自己跑，而不是抢串口；
等待超过 300 秒（可用环境变量 `FLASHGATE_VERIFY_LOCK_WAIT` 调整，单位
秒）才以退出码 6 如实报"台架忙"。所以典型情况下叠加注册不再产生假
故障，但仍有两个代价：每次停止可能串行跑 N 轮验证（N 个注册级别），
且极慢的台架上第二个 hook 可能撞它自己的 480 秒子进程预算——留着叠加
只是为了救急，正常情况下一个级别注册一份就够了。远程台架的泄流预算
会随锁等待配置自动放大（`FLASHGATE_VERIFY_LOCK_WAIT` + 480 秒 verify
预算，默认约 780 秒）：排队等锁的操作也能等到终态快照。代价是
`bench-serve --stop` 或 Ctrl+C 后的排空可能持续十几分钟——期间第二次
Ctrl+C 会被有意忽略（泄流中途杀进程会把烧录器留在孤儿状态），不要
强杀，等它自己退出。
...
/ST-Link，重新构建没有用）——签名的新鲜性无法保证时宁可失败。擦除还
带回读校验：回读"答了错话"（非零，或 rc=0 但截断的读回——后端静默
谎报）返回 5；回读根本跑不起来（工具失败/无数据）属于环境问题返回 6。

verify 返回 6 且提到 probes。0.4.2 起显式要求的探针（--probe /
--all-probes，Stop hook 一直属于这种情况）在串口不可用时不再被静默
跳过，而是直接失败——跑不了的检查不能算通过。接好串口重跑，或者
确实只想要 SWD 启动门时去掉探针要求。
...
- **检查范围声明（coverage 段）**：每份记录自带"这份 PASS 证明了什么"
  的边界声明——验证了什么（逐项列出）、没验证什么（探针覆盖之外的
  一切外设与行为；物理效果——寄存器/串口读回是固件自己的软件观察，
  不是独立物理测量；温度/电压/时序环境条件；探针未计划时显式声明"功能行为完全未验证"；计划了但因前置步骤失败而从未执行时如实声明"探针已计划未执行"（与 skipped_checks 一致）；跑了但全挂时如实声明"探针已跑但无一通过"（与 failed_checks 一致）——三种状态三种说法，不互相误称
  "功能行为完全未验证，仅启动身份"）、档案的附加警示
  （板卡 yaml 的 `coverage.notes` 逐字带入，如 apollo 的"LED 可见性
  未独立观测"）。启动成功永远不会显示成"所有功能已通过"。
